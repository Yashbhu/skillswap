    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-4/5 max-w-5xl bg-white shadow-2xl rounded-3xl p-16 text-center">
        ...
        <Link
          ...
          state={{ allowRealtime }} import { Card, CardContent } from "@/components/ui/card";
            ...
            <motion.div key={user.id} whileHover={{ scale: 1.02 }}>
              <Card className="shadow-md rounded-2xl p-4 bg-indigo-50">
                <CardContent>
                  ...
                  <p className={`mb-1 font-medium ${user.status === 'Active' ? 'text-green-600' : 'text-yellow-600'}`}>Status: {user.status}</p>
                  <p className="text-gray-600 text-sm">Total Bookings: {user.total_bookings}</p>
                  <p className="text-gray-600 text-sm">Pending: {user.pending}</p>
                  <p className="text-gray-600 text-sm">Completed: {user.completed}</p>
